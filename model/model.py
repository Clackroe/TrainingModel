...
class AttnSingleTargetClf(nn.Module):
    ...
    def forward(self, encoder_outputs):
        ...
        self.attn_weights = self.attn(
            encoder_outputs).squeeze().transpose(0, 1)
        ...
        context_vec = torch.bmm(
            self.attn_weights, encoder_outputs_transp).squeeze()
...
def makeContextEncoderInput(utt_encoder_hidden, dialog_lengths, batch_size, batch_indices, dialog_indices):
    ...
    utt_encoder_summed = utt_encoder_hidden[-2,
                                            :, :] + utt_encoder_hidden[-1, :, :]
    ...
    states_dialog_batched = [[None for _ in range(
        dialog_lengths[i])] for i in range(batch_size)]
